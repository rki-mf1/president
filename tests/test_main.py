...
def test_multi_input():
    ...
    tmppath = os.path.splitext(tmpfile)[0]
    president_df = pm.aligner(
        reference, [query], path_out=tmppath, id_threshold=0.9, n_threshold=1.0
    )
    ...
    tmppath = os.path.splitext(tmpfile)[0]
    president_df = pm.aligner(
        reference, [query, query], tmppath, id_threshold=0.9, n_threshold=1.0
    )
...
def test_combined_query_names():
    ...
    tmppath = os.path.splitext(tmpfile)[0]
    president_df = pm.aligner(
        reference, query, tmppath, id_threshold=0.9, n_threshold=1.0
    )

    president_df_sort = president_df[
        ["file_in_query", "query_name", "query_index"]
    ].sort_values("query_index")

    shutil.rmtree(tmppath, ignore_errors=True)

    assert np.all(
        president_df_sort.iloc[0:18]["file_in_query"] == "test_combined.fasta"
    )
    assert np.all(
        president_df_sort.iloc[19:]["file_in_query"] == "test_combined_b.fasta"
    )
